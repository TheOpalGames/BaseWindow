...
import net.theopalgames.basewindow.BaseFontRenderer;
...
public class LibGdxFontRenderer extends BaseFontRenderer
{
    ...
    protected int drawChar(double x, double y, double z, double sX, double sY, char c, boolean depthtest)
    {
        ...
        ((LibGdxWindow)this.home).drawLinkedImage(x, y, z, sX, sY - 0.001, col / 16f, row / 8f, (col + width / 8f) / 16f, 1 / 16f + row / 8f, image, true, depthtest);
        return width;
    }

    public void drawString(double x, double y, double z, double sX, double sY, String s)
    {
        ((LibGdxWindow)this.home).setDrawMode(7, true, true, 6 * s.length());
    ...
    {
        ((LibGdxWindow)this.home).setDrawMode(7, false, true, 6 * s.length());
